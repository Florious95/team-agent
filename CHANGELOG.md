...
- Test suite: 16 pending-contract/flaky tests marked ignored for release determinism (tracked privately)
...
- **Feat/Refactor: message-protocol overhaul M0-M3 — one send surface, sealed identity, everything persisted first.**
  - **`send` surface slimmed with sunset notices.** Canonical usage is now one line: `team-agent send TO MESSAGE... [--workspace] [--team] [--json]`. The positional `TO` accepts the full logical grammar: short id (`w1`, scoped by `--team`), qualified `team/agent`, fully-qualified `workspace::team/agent`, leader form `team/leader`, and comma-list fanout `w1,w2`. Legacy flags (`--to-name`, `--targets`, `--pane`, `--task`, `--watch-result`, `--requires-ack`, `--no-ack`, `--no-wait`, `--timeout`, `--confirm-human`, `--message-id`) keep working through a compatibility window but emit a CommandSpec-driven three-part warning (`deprecated; sunset: next compatibility release; action: ...`) on stderr; `--pane` direct injection is refused with guidance (persist a logical TO instead).
  - **Sender identity sealed (spoof-proof).** `--sender` is gone (unknown flags now fail closed with zero side effects); the persisted sender comes from the framework's trusted runtime identity (typed `TrustedSender`, `TEAM_AGENT_ID` positive source), never from a caller-supplied claim. The MCP send schema likewise drops `sender`/`task_id`/`requires_ack`.
  - **Unified addressing lands one message row before any delivery.** Every accepted logical form funnels into the persisted create-message primitive (store-backed `msg_*` correlation id, one row per recipient); the old `named_send_*`/`pane_send_*` direct-inject paths are deleted. A resolvable worker with a missing pane is accepted and the SAME row advances via the standard delivery blocker (`accepted → queued_pane_missing`), replayed by repair — no synthetic refusals, no bypasses.
  - New permanent contracts: `msg_protocol_m1m3_target_contract` (sealed sender / fail-closed flags / persisted funnel / persist-and-return), `msg_protocol_m2c_to_grammar_contract` (full positional TO grammar matrix), revised `naming_addressing_0545_contract` (canonical `name_not_resolvable` reason, qualified positional accepts+persists).

## 0.5.49

- **Fix: quick-start team identity divergence — one addressable team key across every entry point.** Independent-verifier real-machine finding (O-2, third recurrence of the selector-divergence family): `quick-start` without `--team-id` produced two identities — session/spec used the compiled `TEAM.md name:` while the runtime key fell back to the workspace dir basename — and the three addressing entries then disagreed on the same `--team <displayname>` input (`status` accepted, positional `send` refused `target_not_in_team`, qualified `--to-name` refused `name_not_resolvable`). Fix: quick-start computes ONE canonical team key at entry (explicit `--team-id`/`--name` > compiled spec identity > dir basename fallback) and session/state/runtime-spec-path/worker owner scope all consume it; positional `send` unconditionally forwards `selected.team_key` (never the raw selector) into membership/DB scope; the qualified-name resolver canonicalizes the team token through the same selector (exact-key-wins and fail-closed ambiguity preserved). Legacy diverged workspaces are not migrated; their aliases canonicalize consistently across all entries and `owner_team_id` rows only ever carry the canonical key.
- **Fix: external leader binding lost under the unified key (caught by the release gate).** The rs031 socket contract + a 0.5.47-vs-candidate real-machine comparison showed quick-start run from inside a tmux pane no longer bound the caller pane as `leader_receiver` (state reported `leader_receiver_unbound`). Root: the initial runtime state was seeded without the canonical `team_key`, so the initial owner/receiver binding and the scoped save keyed differently and the binding was dropped. Fix: seed the canonical `team_key` into the initial state before owner binding. Verified: rs031 8/0/1 green, real-machine caller-pane probe binds receiver/owner to the caller pane under the canonical key.
- New permanent contract: `quickstart_team_key_consistency_contract` (canonical-only per constitution review: fresh quick-start single-identity assertion + dual-selector × three-entry consistency matrix + DB owner-scope negative check; legacy diverged coverage carried by lib unit regressions + archived real-machine evidence, honestly deferred per MUST-15).

## 0.5.48

- **Fix: `start-agent --force` duplicate same-role windows + failure-path topology mutation.** User-reported online regression (same agent opened twice: same-name double window/double pane, command still `ok:true`). Root: the Rust cutover dropped Python 0.2.11's force→kill-window→spawn semantics, and the 0.5.46 pre-spawn cohort guard explicitly exempted the fake provider. Fix: the guard is now provider-neutral (provider only decides the command plan, never physical pane uniqueness); same-role cohort observation is fail-closed (`list_targets` errors and Unknown liveness refuse instead of silently passing); spawn identity verification polls bounded (500ms/25ms) for the atomically-returned pane id and precisely `kill-pane`s the just-created pane on any verification/startup failure — a refused start leaves the physical topology byte-identical. `--force` on a live same-role pane is now a zero-mutation refusal; force after the pane died repairs to exactly one pane. Also: `list-panes -F` separator switched to a printable `__TA_FIELD__` sentinel (parser retains tab compatibility). New contracts: `startagent_dup_window_verifier_contract` (real CLI + real tmux, canonical commands), hardened `host_cotenant_death_p0_contract` rollback/observation lanes, spawn retry/rollback unit locks.
- **Fix: `fork-agent` clone unreachable in team scope (false success).** Real-machine first use: `fork-agent --team T` returned `ok:true` with a live pane, but short-name send refused `target_not_in_team`, qualified `--to-name` refused `name_not_resolvable`, and `status --team T` hid the clone — fork registered the clone only in the root projection, never in canonical `teams.<T>.agents`. Fix: fork persists through `StateRepository::save(ForkAgent{team_key,agent_id})` routed to the scoped lifecycle-topology-authority writer (same mechanism as add-agent); team identity is taken once from `SelectedTeam.team_key` (also fixes `TEAM_AGENT_OWNER_TEAM_ID` mis-stamping when forking into a non-active team); a post-save readback gate re-selects the team and verifies the canonical row + pane tuple before returning ok (spawned ≠ registered, fail-closed with full rollback); rollback routes `AgentRollback{Some(team)}` to the scoped deleted-agents writer, preserving sibling teams byte-for-byte. Governance: the two fork direct-save allowlist rows are deleted (BASELINE 70→68). New contracts: `fork_team_scope_verifier_contract` (fork → short send → qualified name → status → retire, claude PATH shim, zero tokens), hardened `med_a7` scoped assertions (root fallback removed), sibling-preservation rollback lane, s1a scoped dispatch expectations.

## 0.5.47

- **Fix: credential redaction at external boundaries — status/MCP/CLI/events/physical logs five outlets.** Introduces a single-entry `redaction.rs` scrubber module (`redact_value`/`redact_str` as the only `pub(crate)` entry points) that recursively traverses `serde_json::Value` (Object/Array/String) to mask sensitive keys (tokens, passwords, secrets, API keys) and URL userinfo (`user:pass@host` → `***:***@host`), preserving key names and URL structure. Applied at all five external boundaries: (1) `status` port JSON output; (2) MCP wire responses; (3) CLI `emit.rs` error output; (4) `event_log.rs` persisted events; (5) `profile_smoke.rs` physical CLI-error log (`safe_error` computed before `fs::write` — zero raw bytes on disk). Idempotent: double-redaction is a no-op. False-positive protection: structural fields (agent IDs, session names, workspace paths) are never masked. New contract: `status_credential_redaction_contract` (RED1 single-entry gate + RED2 structural-field preservation). Reported by remote-control fleet (direct escalation). Note (D-o): ExitCode for unknown subcommand remains Error (1) as of 0.5.45 — see 0.5.45 entry.

## 0.5.46

- **Fix: P0 worker cohort R1 — caller-env teardown isolation + restart/start-agent force-cohort dual-live + codex capture weak attribution + reset without proof + abnormal misclassification + atomic tmux spawn identity.** Five-chain P0 fix cohort plus R1 readiness fix: (1) Caller-env teardown isolation: e2e framework strips `TMUX`/`TEAM_AGENT_*` env vars before test workers, preventing teardown from killing the real team-agent session. (2) Restart/start-agent force-cohort: both paths validate cohort consistency before proceeding, preventing dual-live worker collisions. (3) Codex capture weak attribution: capture path gains positive-evidence attribution guards for codex session matching. (4) Reset without proof: `reset-agent` requires an observable evidence signal before writing reset intent. (5) Abnormal misclassification: abnormal exit classifier gains provider/model context from captured worker record. (R1) Atomic tmux spawn identity: restart now binds state to an atomic tmux spawn identity, preventing readiness_timeout when the tmux pane output identity drifts between spawn and writeback. New contracts: `host_cotenant_death_p0_contract` + R1 spawn identity contracts. Note (D-q): 147 source files exceed the 500-line soft guideline — existing debt, no new violations in this PR.
