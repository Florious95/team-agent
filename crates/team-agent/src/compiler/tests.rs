...
#[test]
fn config_authority_t02_compiled_model_provenance_is_paired_and_no_profile_defaults_hold() {
    for (extra, team_extra, expected_model, expected_source) in [
        ...
        let spec = compile_team(&team).unwrap();
        let agent = &spec.get("agents").unwrap().as_list().unwrap()[0];
        assert_eq!(agent.get("model").and_then(Value::as_str), expected_model);
        assert_eq!(agent.get("model_source").and_then(Value::as_str), Some(expected_source));
        let (team_meta, _) = read_front_matter(&team.join("TEAM.md")).unwrap();
        let dynamic = compile_role_agent(&team.join("agents/worker.md"), &team_meta, &workspace_of(&spec)).unwrap();
        assert_eq!(&dynamic.agent, agent);
    }
}
