...
fn d1_claim_team_owner_includes_os_user() {
    ...
    .unwrap();
    assert!(
        r.ok && r.status == LeaseStatus::Claimed,
        "precondition: vacant acquire"
    );
    ...
    assert!(
        owner.contains_key("os_user"),
        "golden claim-path team_owner carries os_user even when empty; keys={:?}",
...
fn d3_bound_pane_receiver_first_reclaim_from_receiver_pane_is_already_bound() {
    ...
    assert!(
        r.ok,
        "a re-claim from the bound RECEIVER pane (%9) is idempotent ok=true; got {r:?}"
    );
...
fn d4_precheck_epoch_zero_is_falsy_falls_through_to_receiver() {
    ...
    assert_eq!(
        r.status,
        LeaseStatus::Refused,
        "precondition: live owner %1 + no confirm -> force_confirm"
    );
    assert_eq!(
        r.owner_epoch,
        Some(OwnerEpoch(5)),
...
fn d5_success_emits_rebind_applied_and_owner_epoch_advanced_not_claim_applied() {
    ...
    assert!(
        names.iter().any(|n| n == "leader_receiver.rebind_applied"),
        "golden success emits leader_receiver.rebind_applied (:712); got {names:?}"
    );
    assert!(
        names.iter().any(|n| n == "owner_epoch_advanced"),
        "golden success emits owner_epoch_advanced (:721); got {names:?}"
    );
    assert!(
        !names.iter().any(|n| n == "leader_receiver.claim_applied"),
...
fn d6_refusal_emits_leader_receiver_audit_event() {
    ...
    assert_eq!(
        r.reason,
        Some(LeaseReason::NotInTmuxPane),
        "precondition: empty caller -> not_in_tmux_pane"
    );
    let names = event_names(&ws);
...
fn d8_provider_preserved_from_prior_receiver_on_reclaim() {
    ...
    assert!(
        r.ok && r.status == LeaseStatus::Claimed,
        "precondition: dead-owner %1 recover"
    );
    assert_eq!(
        state
...
fn toctou_locked_reread_refuses_owner_epoch_advanced_on_concurrent_bump() {
    ...
    let mut state =
        serde_json::json!({"session_name":"team-agent-x","leader_receiver":{"owner_epoch":3}});
    let r = claim_lease_no_incident(
        &ws,
        &mut state,
        None,
    ...
    assert_eq!(
        r.reason,
        Some(LeaseReason::OwnerEpochAdvanced),
