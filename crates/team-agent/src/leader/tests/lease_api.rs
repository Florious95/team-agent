...
fn attach_leader_errors_when_pane_not_resolvable() {
    ...
    match r {
        Err(LeaderError::Validation(msg)) => {
            assert!(
                msg.contains("%1"),
                "Validation 错误须含目标 pane id,got {msg}"
            );
            assert!(
                msg.contains("not found"),
                "须是 pane-not-found 形态,got {msg}"
            );
        }
    ...
    assert!(
        st.get("team_owner").is_none(),
        "resolve 失败不得落 team_owner"
    );
}
...
fn write_lease_dual_state_persists_canonical_root_only_no_legacy_side_effect() {
    ...
    let ws_state: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(&ws_path).expect("workspace state.json 必须存在"),
    )
    .unwrap();
...
fn detect_dual_state_divergence_reports_diverging_panes() {
    ...
    let snap_dir = crate::model::paths::runtime_dir(&ws)
        .join("teams")
        .join("team-sess");
    ...
    std::fs::write(
        snap_dir.join("state.json"),
        serde_json::to_string(&snap).unwrap(),
    )
    .unwrap();
    let d = detect_dual_state_divergence(&ws, &state)
        .unwrap()
        .expect("分叉 → Some(details)");
    assert_eq!(d["workspace_owner_pane"], serde_json::json!("%1"));
    assert_eq!(d["team_owner_pane"], serde_json::json!("%9"));
    assert_eq!(d["workspace_receiver_pane"], serde_json::json!("%1"));
    assert_eq!(d["team_receiver_pane"], serde_json::json!("%9"));
...
fn r8_requeued_exhausted_watchers_event_payload_golden_shape() {
    ...
    let expected: std::collections::BTreeSet<&str> =
        ["watcher_ids", "count", "trigger"].into_iter().collect();
    ...
    assert_eq!(
        ...
        Some(vec!["w1"]),
        "watcher_ids must be the string list of requeued ids"
    );
    assert_eq!(
        payload.get("count").and_then(|v| v.as_u64()),
        Some(1),
        "count == number of requeued watchers"
    );
    assert_eq!(
...
fn app_server_attach_writes_transport_kind_tuple_and_advances_epoch() {
    ...
    assert_eq!(
        out["leader_receiver"]["transport_kind"],
        serde_json::json!("codex_app_server")
    );
    assert_eq!(
        out["leader_receiver"]["mode"],
        serde_json::json!("codex_app_server")
    );
    assert_eq!(
    ...
    assert_eq!(
        saved["team_owner"]["transport_kind"],
        serde_json::json!("codex_app_server")
    );
