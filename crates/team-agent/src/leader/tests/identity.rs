...
fn leader_start_plan_pins_mode_and_leader_env() {
    ...
    for key in [
        ...
        assert!(
            plan.leader_env.contains_key(key),
            "leader_env 缺导出键 {key}"
        );
    }
    assert_eq!(
...
fn p2_leader_override_reads_override_suffixed_env_var() {
    ...
    assert_eq!(
        id.leader_session_uuid_source,
        LeaderSessionUuidSource::Override
    );
...
fn p2_leader_derived_inputs_read_state_record() {
    ...
    let id = leader_identity_context(&ws, None, Some(&state)).unwrap();
    assert_eq!(
        id.machine_fingerprint, "RECORDED-FP-FROM-STATE",
        "state record fp beats env/hostname"
    );
    assert_eq!(
        id.team_id.as_str(),
        "team-agent-myteam",
        "team_id from state.session_name"
    );
}
...
fn p2_leader_os_user_honors_username_then_empty() {
    ...
    assert_eq!(
        id.os_user, "winuser",
        "USERNAME is the second choice (not LOGNAME)"
    );
}
...
fn p2_leader_detect_divergence_catches_owner_uuid_split() {
    ...
    assert!(
        div.is_some(),
        "owner uuid split (A vs B) with matching panes/epoch must be detected"
    );
}
