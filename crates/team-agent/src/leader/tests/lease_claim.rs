...
fn claim_lease_no_incident_vacant_acquire_advances_epoch_and_writes_canonical_only() {
    ...
    let ws_owner = crate::state::ownership::read_owner_value(&ws_state, &canonical_key)
...
fn claim_lease_no_incident_live_owner_refuses_without_confirm() {
    ...
    assert_eq!(
        r.bound_pane_id,
        Some(PaneId::new("%1")),
        "refusal carries bound pane"
    );
    assert_eq!(
        r.owner_epoch,
        Some(OwnerEpoch(2)),
        "refused → precheck epoch, NOT advanced"
    );
}
...
fn claim_lease_no_incident_already_bound_when_caller_is_bound_pane() {
    ...
    assert_eq!(
        r.owner_epoch,
        Some(OwnerEpoch(1)),
        "already-bound → epoch unchanged"
    );
    assert!(
        r.reason.is_none(),
        "already-bound path carries no acquire reason"
    );
}
...
fn claim_leader_persists_full_tmux_endpoint_when_tmux_tmpdir_differs_from_coordinator() {
    ...
    assert_eq!(
        r.receiver
            .as_ref()
            .and_then(|receiver| receiver.tmux_socket.as_deref()),
        Some(leader_socket),
...
fn claim_leader_already_bound_requires_same_delivery_endpoint_not_only_same_pane_id() {
    ...
    let _e = EnvGuard::apply(&[
        ...
        (
            "TMUX",
            Some("/tmp/ta-current-leader-root/tmux-501/dl2f,12345,0"),
        ),
        ("TMUX_TMPDIR", Some("/tmp/ta-coordinator-root")),
    ]);
...
fn claim_leader_already_bound_normalizes_short_tmux_endpoint_to_full_path() {
    ...
    let _e = EnvGuard::apply(&[
        ...
        (
            "TMUX",
            Some("/tmp/ta-current-leader-root/tmux-501/dl9aa40c88,12345,0"),
        ),
        ("TMUX_TMPDIR", Some("/tmp/ta-coordinator-root")),
    ]);
...
fn second_claim_strips_legacy_top_level_ownership_from_persisted_root() {
    ...
    assert!(
        r.ok,
        "S3-OWNER-002: explicit takeover must succeed; got {r:?}"
    );
    assert_eq!(r.status, LeaseStatus::Claimed);
