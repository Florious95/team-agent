...
fn collect_scoped(
    ...
    if !task_updates.is_empty() {
        let mut task_winners = Vec::new();
        let mut updated = std::collections::BTreeSet::new();
        for (task_id, result_id) in task_updates.iter().rev() {
            if updated.insert(task_id) {
                task_winners.push((task_id.clone(), result_id.clone()));
            }
        }
        #[cfg(test)]
        ...
        state = crate::state::repository::StateRepository::new(&paths.run_workspace).commit(
            ...
            |latest| {
                for (task_id, result_id) in &task_winners {
                    let already_applied = latest
                        .get("tasks")
        ...
        for (task_id, result_id) in &task_winners {
            ...
            if !applied {
                return Err(MessagingError::Validation(format!(
                    "task result projection missing after commit: task_id={task_id} result_id={result_id}"
                )));
            }
        }
        #[cfg(test)]
