...
fn assert_rules_t02_final_mcp(argv: &[String], ws: &Path, agent_id: &str, team: &str) {
    ...
    let path = argument
        .strip_prefix('@')
        .expect("final plan references a file");
    ...
    assert_eq!(
        server["args"],
        serde_json::json!(["mcp-server", "--workspace", ws])
    );
    assert_eq!(
        server["env"]["TEAM_AGENT_WORKSPACE"],
...
fn rules_t02_final_mcp_across_fresh_start_restart_and_add() {
    ...
    assert!(
        health.ok,
        "seeded coordinator must prevent daemon spawn: {health:?}"
    );
    let fresh = RecordingTransport::new();
    quick_start_with_transport_in_workspace(&ws, &team_dir, None, true, Some("cpresume"), &fresh)
        .expect("fresh entry");
    ...
    for single in [true, false] {
        ...
        if single {
            ...
            team_agent::lifecycle::restart_with_transport(&ws, true, Some("cpresume"), &transport)
                .expect("team restart entry");
        }
        let argv = transport.single_spawn().argv;
        assert_rules_t02_final_mcp(&argv, &ws, "worker_a", "cpresume");
        assert_eq!(
            flag_value(&argv, "--resume").as_deref(),
    ...
    team_agent::lifecycle::add_agent_with_transport(
        &team_dir,
        &team_agent::model::ids::AgentId::new("worker_b"),
        &role_file,
        false,
        Some("cpresume"),
        &added,
...
fn rules_t02_non_copilot_writer_keeps_canonical_schema() {
    ...
    let config = team_agent::provider::get_adapter(Provider::ClaudeCode)
        .mcp_config(team_agent::provider::AuthMode::Subscription)
        .unwrap();
...
fn write_copilot_team(ws: &Path, name: &str, tools: &[&str], dangerous: bool) -> PathBuf {
    ...
    let role_bypass = if dangerous { "true" } else { "false" };
    std::fs::write(
        team.join("TEAM.md"),
        format!(
    ...
    fn kill_session(&self, _session: &SessionName) -> Result<(), TransportError> {
        *self.session_present.lock().unwrap() = false;
        self.panes
            .lock()
            .unwrap()
            .retain(|pane| &pane.session != _session);
        Ok(())
    }
...
fn copilot_empty_mcp_list_shim(tag: &str) -> TempWorkspace {
    ...
    #[cfg(unix)]
    {
        ...
        .unwrap();
        std::fs::set_permissions(bin.join("ps"), std::fs::Permissions::from_mode(0o755)).unwrap();
