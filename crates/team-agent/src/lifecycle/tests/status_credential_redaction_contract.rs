...
fn eventlog_producers_share_the_central_writer_and_canonical_schema() {
    ...
    let resume_end = ["pub(crate) fn restart_candidates(", "pub fn restart_candidates("]
        .into_iter()
        .find(|marker| rebuild.contains(marker))
        .expect("restart_candidates source section end");
...
fn run_mcp_status_frames(
    ...
    let mut child = Command::new(crate::lifecycle::tests::test_binary_path())
