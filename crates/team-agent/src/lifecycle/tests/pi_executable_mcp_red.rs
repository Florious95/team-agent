...
use crate::lifecycle::launch::pi_mcp::{
    ...
    write_pi_wrapper, PiAdapterIdentity, PiExecutableChain, PiExecutableFileType, PiWrapperRequest,
};
...
fn adapter_identity() -> PiAdapterIdentity {
    PiAdapterIdentity {
        package_name: "pi-mcp-adapter".to_string(),
        version: "2.30.0".to_string(),
        extension_entry: "./index.ts".to_string(),
        package_json: PathBuf::from(
            "/Users/fixture/.pi/agent/npm/node_modules/pi-mcp-adapter/package.json",
        ),
        index_ts: PathBuf::from(
            "/Users/fixture/.pi/agent/npm/node_modules/pi-mcp-adapter/index.ts",
        ),
...
fn pi_executable_chain_freezes_wrapper_real_binary_catalog_and_plugin_identity() {
    ...
    validate_pi_executable_chain(&chain).expect("the frozen wrapper chain is valid");
    ...
    assert!(validate_pi_executable_chain(&wrong_version).is_err());
    ...
    assert!(validate_pi_executable_chain(&wrong_plugin).is_err());
}

#[test]
fn pi_adapter_detector_requires_exact_2_30_0_entry_and_digest() {
    let exact = adapter_identity();
    validate_pi_adapter_identity(&exact).expect("frozen adapter identity");

    let mut mutations = Vec::new();
    let mut wrong_name = exact.clone();
    wrong_name.package_name = "pi-mcp-adapter-fork".to_string();
    mutations.push(wrong_name);
    let mut wrong_version = exact.clone();
    wrong_version.version = "2.30.1".to_string();
    mutations.push(wrong_version);
    let mut wrong_entry = exact.clone();
    wrong_entry.extension_entry = "./dist/index.js".to_string();
    mutations.push(wrong_entry);
    let mut wrong_package_digest = exact.clone();
    wrong_package_digest.package_json_sha256 = "11".repeat(32);
    mutations.push(wrong_package_digest);
    let mut wrong_index_digest = exact;
    wrong_index_digest.index_ts_sha256 = "22".repeat(32);
    mutations.push(wrong_index_digest);

    for mutation in mutations {
        assert!(
            validate_pi_adapter_identity(&mutation).is_err(),
            "every adapter identity field is load-bearing: {mutation:?}"
        );
    }
}
...
fn pi_wrapper_import_failure_has_no_ambient_merge_fallback() {
    ...
    validate_pi_wrapper_source(&source, &adapter_identity(), &candidate)
        .expect("exact isolated import and candidate");

    let broken = source.replace(
        adapter_identity().index_ts.to_string_lossy().as_ref(),
        "/missing/pi-mcp-adapter/index.ts",
    );
    let error = validate_pi_wrapper_source(&broken, &adapter_identity(), &candidate)
