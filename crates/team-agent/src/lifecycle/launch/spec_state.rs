...
#[cfg(test)]
mod claim_rework_seed_tests {
    use super::*;
    use crate::state::owner_gate::CallerIdentity;

    #[test]
    fn seed_writes_pending_receiver_not_attached() {
        let mut state = serde_json::json!({
            "session_name": "team-seed",
            "workspace": "/tmp/claim-rework-seed"
        });
        ...
        let status = state
            .pointer("/teams/team-seed/leader_receiver/status")
            .or_else(|| state.pointer("/leader_receiver/status"))
            .and_then(serde_json::Value::as_str);
        assert_eq!(status, Some("pending"));
        assert_ne!(status, Some("attached"));
    }
}
