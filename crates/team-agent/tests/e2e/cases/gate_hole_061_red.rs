...
fn tooth_3b_every_mapping_executes_equivalent_argv_in_its_e2e_case() {
        ...
        for case in &entry.cases {
                ...
                .any(|actual| documented_command_matches_argv(&entry.command, actual))
            {
                mismatches.push(format!(
                    "command={:?} case={case:?} observed={actual_argv:?}",
                    entry.command
                ));
            }
        }
    ...
    assert!(
        mismatches.is_empty(),
        "TOOTH-3B RED: mapped E2E cases do not pass equivalent literal argv to `run_ta`:\n{}. \
         Equivalence is exact token-for-token except `<name>` matches exactly one non-empty \
         token and `[name]` matches the literal representative `name`; a final `.` is an \
         ordinary argv token and must match exactly.",
        mismatches.join("\n")
    );
}
...
fn assert_argv_mapping_canary() {
    let source = r#"
fn verifier_mapping_canary() {
    let _ = run_ta(
        &ws,
        &["profile", "show", "codex-default", "--workspace", ".", "--json"],
    );
}
"#;
    let body = test_function_body(source, "verifier_mapping_canary")
        .expect("TOOTH-3B harness canary: find synthetic test body");
    let observed = literal_run_ta_argvs(body);
    assert_eq!(
        observed,
        vec![vec![
            "profile",
        ...
        "TOOTH-3B harness canary: literal run_ta argv extraction changed"
    );
    assert!(documented_command_matches_argv(
