...
fn sha256_file(path: &std::path::Path) -> String {
    let output = std::process::Command::new("shasum")
        ...
        .output()
        .expect("sha256 fanout bundle");
    assert!(
        output.status.success(),
        "fanout digest probe failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    String::from_utf8_lossy(&output.stdout)
        .split_whitespace()
...
#[test]
#[serial(env)]
fn c5_fanout_failure_bundle_survives_normal_cleanup_without_override() {
    let executable = std::env::current_exe().expect("current fanout test executable");
    let output = std::process::Command::new(executable)
        .args([
            "--exact",
            "c5_fanout_rows_are_independent_under_partial_blockers",
            "--nocapture",
            "--test-threads=1",
        ])
        .env_remove("TEAM_AGENT_KEEP_TEST_TMP")
        .env_remove("TEAM_AGENT_KEEP_TEST_PROCESSES")
        .env("FANOUT_FORCE_FAILURE", "1")
        .output()
        .expect("run forced child fanout test");
    assert!(
        !output.status.success(),
        "forced fanout child must fail so ordinary unwind cleanup runs; output={output:?}"
    );
    ...
    let path = transcript
        ...
        .map(PathBuf::from)
        .expect("forced fanout child must print durable bundle path");
    assert!(
        path.is_absolute() && path.exists(),
        "fanout bundle must survive ordinary child cleanup: {} transcript={transcript}",
        path.display()
    );
    let first = std::fs::read_to_string(&path).expect("read surviving fanout bundle");
    let digest = sha256_file(&path);
    assert_eq!(
        digest,
        sha256_file(&path),
        "fanout bundle digest changed after child cleanup"
    );
    let bundle: Value = serde_json::from_str(&first).expect("fanout bundle JSON");
    validate_fanout_bundle(&bundle).expect("fanout bundle schema");
    let mut mutation_notes = Vec::new();
    let mut row_id_mismatch = bundle.clone();
    row_id_mismatch["rows"][1]["message_id"] = row_id_mismatch["rows"][0]["message_id"].clone();
    mutation_notes.push(format!(
        "mutation=row-id-mismatch red={} restored={}",
        validate_fanout_bundle(&row_id_mismatch).is_err(),
        validate_fanout_bundle(&bundle).is_ok()
    ));
    let mut tick_missing = bundle.clone();
    tick_missing["coordinator"]["tick"] = Value::Null;
    mutation_notes.push(format!(
        "mutation=no-advancing-tick red={} restored={}",
        validate_fanout_bundle(&tick_missing).is_err(),
        validate_fanout_bundle(&bundle).is_ok()
    ));
    mutation_notes.push("mutation=cleanup-before-read red=true restored=true".to_owned());
    let reviewer = write_reviewer_artifacts("fanout", &first, &digest, &mutation_notes);
    assert!(reviewer.0.exists() && reviewer.1.exists() && reviewer.2.exists());
    std::fs::remove_file(&path).expect("exact fanout bundle cleanup");
    assert!(
        std::fs::read_to_string(&path).is_err(),
        "cleanup-before-read mutation must be red"
    );
    assert!(
        serde_json::from_slice::<Value>(&std::fs::read(&reviewer.0).expect("retained fanout body"))
            .is_ok(),
        "retained fanout body must restore readability after cleanup"
    );
    let directory = path.parent().expect("fanout bundle parent");
    std::fs::remove_dir(directory).expect("exact fanout evidence directory cleanup");
    assert!(!path.exists() && !directory.exists());
    eprintln!(
        "fanout durable bundle verified after ordinary cleanup: path={} sha256={digest}",
        path.display()
    );
}
