...
//!
//! Triage doc (claimed sole basis): `.team/artifacts/med-triage-fixed-failure-sweep.md` §A-7.
//! ⚠️ 2026-08-23: that file DOES NOT EXIST (证否见本文件第一条 bullet)。⇒ 下面每一条
//! bullet 的需求锚都必须另行溯源，⛔ 不许再把这一行当依据。
//! Python truth source: 0.2.11.
//!
//! - 🔴 2026-08-23 RETIRED: "the label IS the new seat's role". The claim was
//!   sourced from Python `operations.py:315` plus a triage doc
//!   (`.team/artifacts/med-triage-fixed-failure-sweep.md`) that DOES NOT EXIST —
//!   证否用了三种带阳性对照的手段（`ls` / `git log --all --diff-filter=A` /
//!   `find`），见 `.team/artifacts/trace-mismodel-20260823.md` §0.3。
//!   An implementation is not a requirement anchor, and the missing doc is not
//!   one either. The落盘 requirement `wiki/C1/分身与克隆.md:42/:45/:55/:57` says the
//!   opposite: the clone's name is generated by the FRAMEWORK under a uniqueness
//!   rule and ⛔ must not be decided by the user; only "是否唯一" and "是否需要
//!   用户输入" are judgeable, and ⛔ no test may assert the name's concrete shape.
//!   That page's `:59-62` voids, by name, the "the name I hand it becomes its
//!   identity" user behaviour as reverse-engineered from THIS test.
//!   ⇒ The label⇒role assertion is deleted; what remains is the part the same
//!   page does anchor — a clone is a registered, pane-backed, short-name
//!   addressable NEW SEAT.
...
/// A-7: a clone must land as a real, registered, addressable new seat.
///
/// 🔴 2026-08-23 RETIRE: the former `label ⇒ new role` assertion is DELETED.
/// Per `wiki/C1/分身与克隆.md:42/:45/:55/:57` the clone's name is generated by the
/// framework under a uniqueness rule and ⛔ must not be decided by the user; the
/// same page (`:59-62`) explicitly voids the "the name I hand it becomes its
/// identity" behaviour as reverse-engineered from THIS test. What survives here
/// is only what that page does anchor: the clone is **a new seat**, it is
/// **visible** in the canonical team row, it has a **physical pane**, and it is
/// **addressable by short name**.
#[test]
#[serial(a7_mcp)]
fn a7_clone_agent_is_registered_and_addressable() {
...
"#,
    ...
    // `as_agent_id` is still passed because the tool signature requires it
    // (`mcp_server/lifecycle_tools/agent_ops.rs:138-142` takes `&str`, not
    // `Option`). ⚠️ That is a PRODUCT gap against the "the framework generates
    // the unique name" requirement and is deliberately NOT fixed here. What this
    // test must not do is assert that a user-supplied name/label BECOMES the new
    // identity — so every assertion below addresses the seat through the id the
    // tool reports back, never through the literal written above.
    // ⛔ `label` is no longer sent: it has no requirement anchor.
    let call = worker.call_tool(
        "clone_agent",
        json!({
...
body={} raw={}",
    ...
    let new_id = call
        ...
        .and_then(Value::as_str)
        .map(str::to_string)
        .expect("A-7: clone must report the new seat's id; the seat is addressed by THAT id");

    let state = harness.state_value();
    let cloned = state
        .pointer(&format!("/teams/teamA/agents/{new_id}"))
    ...
    let send = worker.call_tool(
        "send_message",
        json!({"to": new_id, "content": "A-7 scoped clone reachability probe"}),
    );
    assert_ne!(
    ...
    assert!(
        rows.iter()
            .any(|row| row.owner_team_id.as_deref() == Some("teamA") && row.recipient == new_id),
