...
fn assign_task_registers_state_and_delegates_message_delivery() {
    ...
    team_agent::state::persist::save_runtime_state(&ws, &json!({
    ...
    assert!(ok.fields.get("task_id").is_none(), "assign_task return is compacted send_message result only");
    assert!(ok.fields.get("to").is_none(), "assign_task must not inject extra `to`");
    ...
    assert_eq!(state["tasks"][1]["id"], json!("task-223"), "new task appends at the end");
    assert!(state["tasks"][1].get("status").is_none(), "missing status is stored as-is");
    assert!(state["teams"]["teamA"].get("status").is_none(), "existing team missing status is not backfilled");
...
fn assign_task_updates_existing_fields_and_uses_golden_content_fallbacks() {
    ...
    let state = team_agent::state::persist::load_runtime_state(&ws).unwrap();
    assert_eq!(state["tasks"].as_array().unwrap().len(), 1, "existing task updates in place");
    ...
    assert_eq!(state["teams"]["teamA"]["tasks"][0]["accepted_result_id"], json!("res-keep"));
    ...
    let ok = tools.assign_task(&fallback, None).expect("json fallback assign ok");
