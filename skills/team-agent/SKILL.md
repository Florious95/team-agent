...
**Operate**
- Dispatch: `team-agent send TO MESSAGE` (`--watch-result` is deprecated). After success, do not poll with `sleep` / `status` / `inbox` / `collect`. TO may be an in-team short name or fully qualified `<workspace>::<team>/<agent>`; use qualified across workspaces.
