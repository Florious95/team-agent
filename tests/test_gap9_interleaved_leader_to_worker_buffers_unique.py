...
class Gap9InterleavedLeaderToWorkerBufferTests(unittest.TestCase):
    def test_gap9_interleaved_leader_to_worker_buffers_unique(self) -> None:
        with tempfile.TemporaryDirectory(prefix="team-agent-gap9-leader-buffers-") as tmp:
            workspace = Path(tmp)
            ...
            with (
                ...
                for index, target in enumerate([agent_ids[0], agent_ids[1], agent_ids[2], agent_ids[0], agent_ids[1]]):
                    result = runtime.send_message(workspace, target, f"leader payload {index}", wait_visible=True)
                    self.assertTrue(result["ok"], result)
                    sends.append(result)

            self.assertEqual(len(buffers), 5)
            self.assertEqual(len(set(buffers)), 5)
            ...
            for index, result in enumerate(sends):
                ...
                self.assertIn(f"leader payload {index}", buffers[buffer_name])
                self.assertIn(["tmux", "paste-buffer", "-t", f"team-gap9:{result['to']}", "-b", buffer_name, "-p"], paste_calls)
